...
void Gui::Init() {
    ...
    // On a HiDPI display the ImGui overlay renders into a framebuffer scaled by the backing scale
    // (e.g. 2x on Retina), but glyphs would be rasterized at the logical point size and stretched up
    // -> fuzzy text. Rasterize at the display's backing scale (mDpiScale, set by Fast3dGui::Init before
    // this runs) via RasterizerDensity so glyphs stay crisp. mDpiScale is 1.0 on standard-DPI displays.
